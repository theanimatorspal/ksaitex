...
% \MAGIC{ shloka_begin, label='श्लोक सुरु', tab='ढाँचा', pairing='begin', group='shloka', args='align:select,alternating,left,right,center:alternating', command='\begin{shloka}[VAR_align]' }
...
% \VAR{ khanda_size, default='\Huge', tab='लेआउट', label='खण्डको आकार', type='select', options='\normalsize|\large|\Large|\huge|\Huge' }
% \VAR{ khanda_align, default='Left', tab='लेआउट', label='खण्डको पङ्क्ति मिलाउने', type='select', options='Left|Center|Right' }
% \VAR{ upakhanda_size, default='\huge', tab='लेआउट', label='उपखण्डको आकार', type='select', options='\normalsize|\large|\Large|\huge|\Huge' }
% \VAR{ upakhanda_align, default='Left', tab='लेआउट', label='उपखण्डको पङ्क्ति मिलाउने', type='select', options='Left|Center|Right' }
% \VAR{ upaupakhanda_size, default='\Large', tab='लेआउट', label='उप-उपखण्डको आकार', type='select', options='\normalsize|\large|\Large|\huge|\Huge' }
...
\newcommand{\khanda}[1]{
    \markboth{#1}{}
    \vspace{0.8cm}
    \noindent \BLOCK{ if khanda_align == 'Right' or khanda_align == 'Center' }\hfill\BLOCK{ endif }{\VAR{khanda_size} \color{theme} \textbf{#1}}\BLOCK{ if khanda_align == 'Left' or khanda_align == 'Center' }\hfill\BLOCK{ endif } \par
...
\newcommand{\upakhanda}[1]{
    \vspace{0.6cm}
    \noindent \BLOCK{ if upakhanda_align == 'Right' or upakhanda_align == 'Center' }\hfill\BLOCK{ endif }{\VAR{upakhanda_size} \color{theme} \textbf{#1}}\BLOCK{ if upakhanda_align == 'Left' or upakhanda_align == 'Center' }\hfill\BLOCK{ endif } \par
...
\newcommand{\upaupakhanda}[1]{
    \vspace{0.4cm}
    \noindent \BLOCK{ if upaupakhanda_align == 'Right' or upaupakhanda_align == 'Center' }\hfill\BLOCK{ endif }{\VAR{upaupakhanda_size} \color{theme} \textbf{#1}}\BLOCK{ if upaupakhanda_align == 'Left' or upaupakhanda_align == 'Center' }\hfill\BLOCK{ endif } \par
...
\newenvironment{shloka}[1][alternating]{
    ...
    \ifthenelse{\equal{\shlokaalign}{alternating}}{
        \renewcommand{\\}{
            \stepcounter{shlokaline}
            \ifodd\value{shlokaline}
                \hfill\par\noindent\hfill
            \else
                \par\noindent
            \fi
